/* ══════════════════════════════════════════════════════════════════════
   join-video.js — วิดีโอ YouTube ที่เล่นเองในพาเนลซ้ายของหน้าสมัครตัวแทน
   ...
   ───────────────────────
   1) ไม่โหลด iframe ตอนเปิดหน้า — YouTube ลากสคริปต์มาราว 1–1.5MB
      จึงรอจน IntersectionObserver บอกว่าเลื่อนใกล้ถึงแล้วค่อยสร้าง
   ...
   ใส่รหัสวิดีโอที่เดียว: แอตทริบิวต์ data-yt ของ .jvid ใน join.html
   ══════════════════════════════════════════════════════════════════════ */
(function () {
  "use strict";

  var box = document.querySelector(".jvid");
  if (!box) return;

  var id = (box.getAttribute("data-yt") || "").trim();
  /* ยังไม่ได้ใส่รหัสวิดีโอ — ซ่อนกล่องไปเลย ดีกว่าโชว์กรอบว่าง */
  if (!id || id === "VIDEO_ID") {
    box.style.display = "none";
    return;
  }

  var frame = box.querySelector(".jvid-frame");
  var ph = box.querySelector(".jvid-ph");
  var snd = box.querySelector(".jvid-sound");
  ...
  /* ภาพปกดึงจาก YouTube ตามรหัสวิดีโอ ไม่ต้องอัปโหลดรูปเอง
     ...
     ตั้ง src จากสคริปต์ ไม่ใส่ใน HTML เพราะ <img src=""> ทำให้บางเบราว์เซอร์
     ยิงคำขอซ้ำไปที่ URL ของหน้าเอง */
  ...
  /* รอให้หน้าโหลดเสร็จก่อนค่อยแตะ YouTube — iframe ลากของมาราว 1–1.5MB
     ถ้าปล่อยให้เริ่มตั้งแต่ระหว่างโหลด มันจะแย่งแบนด์วิดท์กับรูปและฟอนต์
     ของหน้าเอง ดันเวลาที่ผู้ใช้เห็นเนื้อหาให้ช้าลง */
  ...
  function build(autoplay, startMuted) {
    ...
    iframe.title = box.getAttribute("data-title") || "วิดีโอแนะนำการสมัครเป็นตัวแทน";
